...
def generate_pixels():
    ...
    for i in range(SLIC_ITERATIONS):
...
def display_contours(color):
    is_taken = numpy.zeros(img.shape[:2], bool)
    contours = []
    ...
    for i in range(SLIC_width):
        ...
        #end
    #end
    new_img = numpy.zeros(numpy.shape(img))
    new_img[new_img == 0] = 1
    for i in range(len(contours)):
        img[contours[i][0], contours[i][1]] = color
        new_img[contours[i][0], contours[i][1]] = color
    #end
    return new_img
#end
...
cons = 50
super = 500
step = int((img.shape[0]*img.shape[1]/int(super))**0.5)
SLIC_m = int(cons)
SLIC_ITERATIONS = 4
...
# main
def re_run(): # Used by slic_imp to recalculate the superpixels
    global img, SLIC_height, SLIC_width, SLIC_labimg, SLIC_distances, SLIC_clusters, SLIC_center_counts, SLIC_centers
    img = cv2.imread("test.png")
    SLIC_height, SLIC_width = img.shape[:2]
    SLIC_labimg = cv2.cvtColor(img, cv2.COLOR_BGR2LAB).astype(numpy.float64)
    SLIC_distances = 1 * numpy.ones(img.shape[:2])
    ...
    create_connectivity()
    calculate_centers()
    contours = numpy.array(display_contours([0.0, 0.0, 0.0]))
    contours = numpy.uint8(contours*255)
    return contours
